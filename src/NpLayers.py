...
class HiddenLayer():
    def __init__(self, n_inputs, n_outputs):
        ...
        self.W = np.random.uniform(-1, 1, (self.n_outputs, self.n_inputs))
...
class Network():
    ...
    def train_L(self, data, init_weights=None, tol=0.0001):
        if init_weights is None:
            init_weights = self.get_network_weights()
        print "L-BFGS-B training..."
    ...
    def train_adagrad(self, data, init_weights=None, maxfun=5):
        ...
        I = 1.0
        print "adagrad training with eta0="+str(eta0)+"..."
    ...
    def train_sgd(self, data, init_weights=None, maxfun=3):
        if init_weights is None:
            print 'using network weights...'
            weights = self.get_network_weights()
        else:
        ...
        beta = 0.0
        mom = 0.0
        print "sgd training with eta0="+str(eta0)+" and beta="+str(beta)+"..."
        for i in range(maxfun):     
            print "iteration "+str(i)
            shuffle(ids)
            for d_id in ids:
                eta_t = eta0 / np.sqrt(len(data)*i+d_id+1) 
                grad = self.get_gradient(weights, data[d_id:d_id + 1]) 
                weights += beta*mom - np.multiply(eta_t, grad)
                mom = beta*mom - np.multiply(eta_t, grad)
            print self.get_cost(weights, data)
        return weights
